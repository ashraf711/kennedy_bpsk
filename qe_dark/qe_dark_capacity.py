...
def optimize_p_for_fixed_beta(N, beta, tol=1e-12, maxit=200, eta = 1.0, lambd = 0.0):
    """
...
def maximize_I_ml(N, restarts=5, steps=1500, lr=0.0001, seed=0, device="cuda:0", init = None, patience = 300, verbose = True, eta = 1.0, lambd = 0.0):
    """
    ...
    for r in range(restarts):
        ...
        for t in range(steps):
            ...
            I = I_kennedy_t(N_t, beta, p, eta = eta, lambd=lambd)
            loss = -I  # maximize I
...
def run_capacity_sweep(N_vals, device, eta, lambd, tag, verbose=True):
    """
    Runs your capacity-maximization pipeline for a given (eta, lambd) and saves
    CSV + plots with filenames suffixed by _{tag}.
    """
    # Baselines that don't depend on detector model
    C_inf = holevo_C_infty(N_vals)
    C1 = dolinar_C1(N_vals)

    # Kennedy β=0 with p optimized (non-ideal detector)
    C_kennedy_beta0, p_beta0 = [], []
    for N in N_vals:
        p_opt = optimize_p_for_fixed_beta(N, beta=0.0, eta=eta, lambd=lambd)
        I_val = I_kennedy_np(N, beta=0.0, p=p_opt, eta=eta, lambd=lambd)
        C_kennedy_beta0.append(I_val)
        p_beta0.append(p_opt)
    C_kennedy_beta0 = np.array(C_kennedy_beta0)
    p_beta0 = np.array(p_beta0)

    # Kennedy with (β,p) jointly optimized (non-ideal detector)
    C_kennedy_ml, beta_star, p_star = [], [], []
    init = None
    for idx, N in enumerate(N_vals):
        best_I, best_beta, best_p = maximize_I_ml(
            float(N),
            restarts=6, steps=1800, lr=1e-4, seed=1234,
            device=device, init=init, patience=400, verbose=verbose,
            eta=eta, lambd=lambd
        )
        ...
        if verbose and idx % 25 == 0:
            print(f"[{tag}] Progress: {idx}/{len(N_vals)} (N={N:.4f})")

    C_kennedy_ml = np.array(C_kennedy_ml)
    beta_star = np.array(beta_star)
    p_star = np.array(p_star)

    # --- Save CSV (suffix by tag) ---
    csv_name = f"capacity_results_{tag}.csv"
    with open(csv_name, "w", newline="") as f:
        ...
        for i in range(len(N_vals)):
            w.writerow([float(N_vals[i]),
                        float(np.exp(-2*N_vals[i])),
                        float(C_inf[i]),
                        float(C1[i]),
                        float(C_kennedy_beta0[i]),
                        float(C_kennedy_ml[i]),
                        float(p_beta0[i]),
                        float(beta_star[i]),
                        float(p_star[i]),
                        float(eta), float(lambd)])
    ...
    plt.plot(x_plot, C_inf[order], label="Holevo $C_\\infty(N)$", lw=2.5)
    plt.plot(x_plot, C1[order], "--", label="Dolinar $C_1(N)$", lw=2.5)
    plt.plot(x_plot, C_kennedy_beta0[order], "-.", label=f"Kennedy (β=0), p opt [{tag}]", lw=2.5)
    plt.plot(x_plot, C_kennedy_ml[order], ":", label=f"Kennedy (β,p) opt [{tag}]", lw=3.0)
    plt.xlabel(r"$e^{-2N}$")
    plt.ylabel("Capacity (bits / symbol)")
    plt.title(f"BPSK Capacity [{tag}]  η={eta}, λ={lambd}")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"results/capacity_curves_{tag}.png", dpi=160)
    plt.close()

    # β* and p* vs N
    fig, ax = plt.subplots(1, 2, figsize=(10, 4))
    ax[0].plot(N_vals, beta_star, lw=2.0)
    ax[0].set_xlabel("N = |α|^2"); ax[0].set_ylabel("β* (ML)")
    ax[0].set_title(f"β* vs N [{tag}]")
    ax[0].grid(True, alpha=0.3)
    ax[1].plot(N_vals, p_star, lw=2.0)
    ax[1].set_xlabel("N = |α|^2"); ax[1].set_ylabel("p* (ML)")
    ax[1].set_title(f"p* vs N [{tag}]")
    ax[1].grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"results/opt_params_{tag}.png", dpi=160)
    plt.close()

    print(f"[{tag}] Saved:")
    print(f"  {csv_name}")
    print(f"  capacity_curves_{tag}.png")
    print(f"  opt_params_{tag}.png")


def plot_combined_beta_star(configs):
    plt.figure(figsize=(8, 5))
    for _, _, tag in configs:
        df = pd.read_csv(f"capacity_results_{tag}.csv")
        plt.plot(df["N"], df["beta_star"], lw=2, label=f"{tag}")
    plt.xlabel("Photon number N = |α|²")
    plt.ylabel("β* (optimal displacement)")
    plt.title("Comparison of β*(N) under different detector conditions")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig("results/beta_star_all_conditions.png", dpi=160)
    plt.show()
    print("Saved: beta_star_all_conditions.png")
...
def main():
    ...
    # Your list of detector configs
    configs = [
        (1.0, 0.0,   "ideal"),
        (0.7, 0.0,   "eta0p7"),
        (0.7, 1e-3,  "eta0p7_dc1e-3"),
    ]

    for eta, lambd, tag in configs:
        run_capacity_sweep(N_vals, device, eta, lambd, tag, verbose= True)
    
    
    plot_combined_beta_star(configs)
